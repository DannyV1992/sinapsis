...
│   │   │   └── layout.tsx
│   │   ├── herramientas/
│   │   │   ├── page.tsx        # Herramientas interactivas con navegación en 2 niveles: Área (categoría) + Modelo (sub-herramienta). 3 categorías: Respiración (4-7-8 / Respiración cuadrada — anillo via @keyframes breath-ring + key={faseIdx}, useReducer, prop tecnicaInicial sin selector interno) · Emociones (Rueda de Emociones / Rueda Gloria Willcox / Rueda Plutchik) · Anclaje (Grounding 5-4-3-2-1 / Escáner Corporal). Rueda Plutchik: SVG forma de flor, disco completo — cuñas de díadas + hojas curvas; 8 emociones × 3 bandas radiales, punta convergente que sobresale del anillo; helpers ptDeg/petalHalfWidth/bandPath/wedgePath/labelPos/hexToRgb/mixColor/rgbToHsl/hslToCss/intensifyColor/blendHex; constantes WHEEL_CX/CY, R_CORE/R_INTENSE/R_PRIMARY/R_MILD/R_WEDGE, MAX_HALF; tipo NivelEmocion {similares, sensaciones, mensaje, proposito, opuesto}; EmocionP con dataPrimaria/dataIntensa/dataLeve; diadasP con similares/sensaciones/mensaje/proposito/explicacion; estado selIdx+selNivel+selDiada — tocar pétalo muestra panel con 4 campos + opuesto, tocar cuña de díada muestra panel con 5 campos + combinación; selección mutuamente excluyente; AnimatePresence para ambos paneles. Rueda Ekman: implementada (RuedaEkman) pero no expuesta en navegación — SVG 6 sectores, panel detalle con descripción + señales físicas. Rueda de Emociones (RuedaEmocioneWheel): componente data-driven reutilizable, acepta props `data` y `infoTexto`; sunburst de 3 anillos concéntricos; helper robSector(r1,r2,startDeg,endDeg), constantes ROB_CX/CY, ROB_R1/R2/R3, mapa ROB_COLORS por núcleo con 3 tonos (l1/l2/l3); selección jerárquica por selKey ("ci"/"ci-l2i"/"ci-l2i-l3i") con isDimmed; etiquetas radiales rotadas (rotLabel); panel de detalle con desc + chips de hijos. Rueda Willcox (Gloria Willcox 1982): usa RuedaEmocioneWheel con willcoxData — 6 primarias (Enfado/Miedo/Felicidad/Calma/Fuerza/Tristeza) × 6 secundarias × 1 terciaria por secundaria (estructura exacta del original: anillo exterior tiene 1 emoción por sector, no 2); tipos EmocioneL1/L2/L3 con nombre/desc/hijos anidados. Rueda Neurológica: usa RuedaEmocioneWheel con emocioneData — 6 emociones básicas con sustrato neurológico documentado (Alegría/Ira/Asco/Tristeza/Miedo/Sorpresa) en pares opuestos complementarios (Alegría↔Tristeza amarillo↔azul, Ira↔Miedo rojo↔violeta, Asco↔Sorpresa naranja↔verde); × 6 secundarias × 2 terciarias; descripciones L1 con base neurológica (estructuras cerebrales + neurotransmisores). Orden de navegación: Rueda Neurológica → Rueda de Plutchik → Rueda de Gloria Willcox. Todos los textos SVG en negro (#1a1a1a) sin negrita. Navegación: label "Área" + chips categoría (motion layoutId chip-bg-categoria) + label "Modelo" + chips sub-herramienta; al cambiar categoría se selecciona primer modelo automáticamente — chips con estilo empresas
│   │   │   └── layout.tsx
│   │   └── descargas/
