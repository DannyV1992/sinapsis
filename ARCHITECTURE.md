...
│   │   │   └── layout.tsx
│   │   ├── herramientas/
│   │   │   ├── page.tsx        # Herramientas interactivas con navegación en 2 niveles: Área (categoría) + Modelo (sub-herramienta). 3 categorías: Respiración (4-7-8 / Respiración cuadrada — anillo via @keyframes breath-ring + key={faseIdx}, useReducer, prop tecnicaInicial sin selector interno) · Emociones (Rueda de Emociones / Rueda Plutchik / Modelo Ekman) · Anclaje (Grounding 5-4-3-2-1 / Escáner Corporal). Rueda Plutchik: SVG forma de flor, disco completo — cuñas de díadas + hojas curvas; 8 emociones × 3 bandas radiales, punta convergente que sobresale del anillo; helpers ptDeg/petalHalfWidth/bandPath/wedgePath/labelPos/hexToRgb/mixColor/rgbToHsl/hslToCss/intensifyColor/blendHex; constantes WHEEL_CX/CY, R_CORE/R_INTENSE/R_PRIMARY/R_MILD/R_WEDGE, MAX_HALF; tipo NivelEmocion {similares, sensaciones, mensaje, proposito, opuesto}; EmocionP con dataPrimaria/dataIntensa/dataLeve; diadasP con similares/sensaciones/mensaje/proposito/explicacion; estado selIdx+selNivel+selDiada — tocar pétalo muestra panel con 4 campos + opuesto, tocar cuña de díada muestra panel con 5 campos + combinación; selección mutuamente excluyente; AnimatePresence para ambos paneles. Rueda Ekman: SVG 6 pétalos (Alegría/Tristeza/Miedo/Asco/Ira/Sorpresa), constantes EKM_CX/CY/R_INNER/R_OUTER/R_TIP, función ekmanPetalPath con punta real convergente, panel detalle con descripción + señales físicas. Rueda de Emociones (RuedaEmocioneWheel): sunburst de 3 anillos concéntricos basado en la rueda clásica de 6 emociones (Ira/Disgusto/Tristeza/Felicidad/Sorpresa/Miedo), cada núcleo con 6 emociones medias y 12 externas (2 por media); data-driven vía emocioneData (tipos EmocioneL1/L2/L3 con nombre/desc/color/hijos anidados) — la geometría se adapta al número de núcleos; helper robSector(r1,r2,startDeg,endDeg), constantes ROB_CX/CY, ROB_R1/R2/R3, mapa ROB_COLORS por núcleo con 3 tonos (l1/l2/l3); selección jerárquica por selKey ("ci"/"ci-l2i"/"ci-l2i-l3i") con isDimmed para atenuar ramas no seleccionadas; etiquetas radiales rotadas (rotLabel); fontSize L1=13, L2=10, L3=10; panel de detalle con desc + chips de hijos. Todos los textos SVG en negro (#1a1a1a) sin negrita. Navegación: label "Área" + chips categoría (motion layoutId chip-bg-categoria) + label "Modelo" + chips sub-herramienta; al cambiar categoría se selecciona primer modelo automáticamente — chips con estilo empresas
│   │   │   └── layout.tsx
│   │   └── descargas/
